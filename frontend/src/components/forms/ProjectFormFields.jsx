...
const ProjectFormFields = ({ control, errors, projectmanager = [], employees = [], statusOptions = [] }) => {
    const employeeOptions = useMemo(() => (employees ?? []).map((e) => ({ id: e.id, name: `${e.first_name ?? ""} ${e.last_name ?? ""}`.trim() })), [employees]);
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Form.Select {...field} isInvalid={!!errors.status}>
                                ...
                                {(statusOptions ?? []).map((o) => <option key={o.id ?? o.name} value={o.id ?? o.name}>{o.name}</option>)}
                            </Form.Select>
                        )}
                    />
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Form.Select
                                multiple
                                value={field.value || []}
                                onChange={(e) => field.onChange(Array.from(e.target.selectedOptions).map((opt) => opt.value))}
